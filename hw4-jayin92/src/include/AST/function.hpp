...
class FunctionNode final : public AstNode {
  ...
  public:
    ...
    const char *getRetTypeCString() const { return m_ret_type->getPTypeCString(); }
    PTypeSharedPtr getRetTypeSharedPtr() const { return std::shared_ptr<PType>(m_ret_type.get()); }
    const char *getParametersCString() const;
    const char *getPrototypeCString() const;
    DeclNodes &getParameters() { return m_parameters; }
    std::shared_ptr<CompoundStatementNode> getBodySharedPtr() const {
